...
interface Props {
  articleData: ArticleModel | Article;
}

const ProceedToPublishBtn = ({ articleData }: Props) => {
  const session = useSession();
  const router = useRouter();
  const [isLoading, setIsLoading] = useState<boolean>(false);

  // publish Handler
  const proceedToPublish = async (statusValue: string) => {
    const {
      title,
      desc,
      articleContentOne,
    ...
    try {
      ...
      if (error) {
        ...
        console.log(error);
        toast.error(`Error in publishing Article.${errMsg}`);
        return;
      }
      if (data) {
        toast.success("sucessfully published article");
      ...
      console.log(error);
      toast.error(`Error in publishing Article.`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <button
      ...
      onClick={() => proceedToPublish("published")}
      disabled={isLoading}
    >
      <h1 className="text-sm">Procced To Publish</h1>
      {isLoading && (
        <iframe
...
export default ProceedToPublishBtn;
