...
const SpotLightTogglerButton = ({ isDjOfTheWeek, articleId }: Props) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!isDjOfTheWeek) {
        ...
        const { data, error } = await deleteDjsoFTheWeek(
          articleId,
          sessionData?.user.token || ""
        );
        if (error) {
...
export default SpotLightTogglerButton;
