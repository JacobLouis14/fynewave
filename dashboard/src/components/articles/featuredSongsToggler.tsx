...
const FeaturedSongsTogglerButton = ({ isSongOfTheWeek, articleId }: Props) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (isSongOfTheWeek) {
        const { data, error } = await deleteSongsOfTheWeek(
          articleId,
          sessionData?.user.token || ""
        );
        if (error) {
          console.log(error);
          toast.error("error in removing from songs of the week");
          return;
        }
        if (data) {
          toast.success("sucessfully remove songs of the week");
          cancelHandler();
          return;
        }
      } else {
        ...
        if (error) {
          console.log(error);
          toast.error("error in updating songs of the week");
          return;
        }
        if (data) {
          toast.success("sucessfully update songs of the week");
          cancelHandler();
          return;
        }
      }
...
export default FeaturedSongsTogglerButton;
